import { CONFIG, CATEGORY_META } from './core/config.js';
import { 
  productos, setProductos,
  filtrosCategorias, setFiltrosCategorias,
  filtrosMarcas, setFiltrosMarcas,
  filtrosTiposPiel, setFiltrosTiposPiel,
  filtrosObjetivosSalud, setFiltrosObjetivosSalud,
  soloOfertas, setSoloOfertas,
  busqueda, setBusqueda,
  productosFiltradosGlobal, setProductosFiltradosGlobal,
  currentPage, setCurrentPage,
  ITEMS_PER_PAGE,
  observerInfiniteScroll, setObserverInfiniteScroll,
  filterAccordionCollapsed,
  carrito, setCarrito
} from './core/state.js';

// Bindings to window for legacy inline HTML onclicks
window.bellanovaState = { setProductos, setCarrito, setCurrentPage };
/* ═══════════════════════════════════════════════════════════════
      ...
      checkbox.addEventListener('change', (e) => {
        ...
        if (type === 'category') {
          if (checked) {
            ...
            setFiltrosCategorias(filtrosCategorias.filter(x => x !== val));
          }
        } else if (type === 'brand') {
          if (checked) {
            if (!filtrosMarcas.includes(val)) filtrosMarcas.push(val);
          } else {
            setFiltrosMarcas(filtrosMarcas.filter(x => x !== val));
          }
        } else if (type === 'skin') {
          if (checked) {
            if (!filtrosTiposPiel.includes(val)) filtrosTiposPiel.push(val);
          } else {
            setFiltrosTiposPiel(filtrosTiposPiel.filter(x => x !== val));
          }
        } else if (type === 'goal') {
          if (checked) {
            if (!filtrosObjetivosSalud.includes(val)) filtrosObjetivosSalud.push(val);
          } else {
            setFiltrosObjetivosSalud(filtrosObjetivosSalud.filter(x => x !== val));
          }
        }
  ...
  function buscarProductos(query) {
    setBusqueda(query.toLowerCase().trim());
    filtrarProductos();
    actualizarUrlParameters();
  }
  ...
  function filtrarProductos() {
    ...
    setProductosFiltradosGlobal(resultado);
    setCurrentPage(1);
  ...
  function renderizarChipsFiltros() {
    ...
    const crearChip = (valor, tipo) => {
      ...
      `;
      chip.addEventListener('click', () => {
        if (tipo === 'categoria') setFiltrosCategorias(filtrosCategorias.filter(v => v !== valor));
        if (tipo === 'marca') setFiltrosMarcas(filtrosMarcas.filter(v => v !== valor));
        if (tipo === 'piel') setFiltrosTiposPiel(filtrosTiposPiel.filter(v => v !== valor));
        if (tipo === 'salud') setFiltrosObjetivosSalud(filtrosObjetivosSalud.filter(v => v !== valor));
        if (tipo === 'ofertas') {
  ...
  async function init() {
    ...
    if (isProductPage) {
      ...
      if (isCatalogPage) {
        ...
        const params = new URLSearchParams(window.location.search);
        if (params.has('cat')) {
          setFiltrosCategorias(params.get('cat').split(',').map(s => s.trim()).filter(Boolean));
        }
        if (params.has('brand')) {
          setFiltrosMarcas(params.get('brand').split(',').map(s => s.trim()).filter(Boolean));
        }
        if (params.has('skin')) {
          setFiltrosTiposPiel(params.get('skin').split(',').map(s => s.trim()).filter(Boolean));
        }
        if (params.has('goal')) {
          setFiltrosObjetivosSalud(params.get('goal').split(',').map(s => s.trim()).filter(Boolean));
        }
        ...
        if (params.has('search')) {
          ...
          if (searchQuery === 'focus') {
            ...
            setBusqueda(searchQuery.trim().toLowerCase());
...
// Register functions globally for inline HTML events
window.abrirProductoRapido = abrirProductoRapido;
